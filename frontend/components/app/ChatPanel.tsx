...
/**
 * ChatPanel — floating chat panel with framer-motion animations.
 * Shows chat messages from desktop app via Pear P2P.
 * If P2P is not connected, shows a message prompting to connect.
 */
...
export default function ChatPanel({ open, onClose }: ChatPanelProps) {
  const { p2pConnected } = useWalletMode();
  const [messages, setMessages] = useState<ChatMessage[]>([]);
  const [input, setInput] = useState("");
  const messagesEndRef = useRef<HTMLDivElement>(null);

  // Poll for messages every 5 seconds (only if P2P connected)
  useEffect(() => {
    if (!open || !p2pConnected) return;
  ...
  const handleSend = async () => {
    if (!input.trim() || !p2pConnected) return;
    try {
      await cli.chat.send(input.trim());
      setInput("");
          ...
          {!p2pConnected ? (
            <div className="flex flex-col items-center justify-center h-64 gap-3 px-4">
              ...
              <p className="text-xs text-gray-400 text-center m-0">
                Reconnect with an invite code to enable team chat.
              </p>
            </div>
          ) : (
            <>
                    ...
                    {messages.map((msg) => (
                      <div key={msg.id} className="rounded bg-gray-50 p-2">
                        <div className="flex items-center gap-2 mb-1">
                          <span className="text-xs font-semibold text-gray-700">{msg.info?.name || "Unknown"}</span>
                          <span className="text-[10px] text-gray-400">
                            {msg.info?.at ? new Date(msg.info.at).toLocaleTimeString() : ""}
                          </span>
                        </div>
