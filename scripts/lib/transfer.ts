 ...
 * call in 03-transfer-amulet.ts is unchanged.
 *
 * Mirrors desktop-app/src/main/transfer.ts (same DevNet endpoints).
 */
 ...
 * `templateId` slot in the ExerciseCommand — Canton accepts either a
 * fully-qualified template id OR an interface id (with leading `#`) in
 * this slot, and the interface id is stable across Splice deployments.
 */
 ...
 * to Amulet) AND the createdEventBlob. The SDK does the same under
 * the hood but also reaches into the CIP-0056 metadata registry
 * (which 404s on FiveNorth) — going through the ledger JSON API
 * directly skips that path.
 */
...
export async function buildTransferCommand(
  ...
  //      }
  const contextValues = (ctx.choiceContextData?.values ?? {}) as Record<
    string,
    unknown
  >;
