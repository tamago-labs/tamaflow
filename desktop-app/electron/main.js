...
if (process.env.TAMAFLOW_REMOTE_DEBUGGING_PORT) {
  app.commandLine.appendSwitch(
    'remote-debugging-port',
    String(process.env.TAMAFLOW_REMOTE_DEBUGGING_PORT)
  )
}
if (process.env.TAMAFLOW_NO_SANDBOX === '1') {
...
// `start:guest` uses `--storage ./tmp-tamaflow-guest`. Electron's
...
function getWorker(specifier) {
  ...
  // workers/room-entry.js — so we extend the base argv with
