  ...
  return (
    <div className="relative w-full h-full overflow-hidden flex flex-col">
      ...
      <div className="flex items-center justify-between px-3 py-2 bg-white border-b border-gray-200 z-200 flex-shrink-0" style={{ zIndex: 200 }}>
        ...
        {/* Right: Preview + Start/Stop/Delete */}
        <div className="flex items-center gap-2">
          {flowStatus === 'draft' && (
            <>
      ...
      <div className="flex-1 min-h-0 relative">
        ...
        {/* Bottom-right save badge */}
        {badgeLabel && (
          <div className="absolute bottom-4 right-4 z-200" style={{ zIndex: 200 }}>
            <span className={`px-3 py-1.5 text-[10px] font-mono font-bold uppercase tracking-wider rounded shadow-sm ${saveStatus === 'saving' ? 'text-blue-600 bg-blue-50 border border-blue-200' : saveStatus === 'saved' ? 'text-green-600 bg-green-50 border border-green-200' : 'text-red-600 bg-red-50 border border-red-200'}`}>
              {badgeLabel}
            </span>
          </div>
        )}
      ...
      <div style={{ overflowY: 'auto', flex: 1 }}>
        {sorted.length === 0 ? (
          ...
          <table className="w-full border-collapse">
            <thead><tr style={{ background: '#f7f7fc' }}>
  ...
  return (
    <div>
      <div className="mb-6 flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          <button type="button" onClick={onImport} className="flex items-center gap-1.5 px-3 py-2 text-xs font-bold text-gray-700 bg-white border border-gray-200 rounded-md hover:bg-gray-50 transition"><Download size={14} />Import</button>
          <button type="button" onClick={onCreate} className="flex items-center gap-1.5 px-3 py-2 text-xs font-bold text-white bg-[#1A1AE8] rounded-md hover:bg-[#1515c0] transition"><Plus size={14} />New Flow</button>
        </div>
      </div>
      <div className="overflow-hidden rounded-md border border-gray-200 bg-white">
        ...
        {!hasAny && <div className="flex flex-col items-center gap-3 py-16 text-center"><div className="flex h-10 w-10 items-center justify-center rounded-full border border-gray-200 bg-gray-50 text-gray-400"><Plus size={20} /></div><p className="m-0 font-sans text-sm font-medium text-gray-900">No flows yet</p><p className="m-0 max-w-sm font-sans text-xs text-gray-400">Create your first payroll flow to get started.</p><button type="button" onClick={onCreate} className="mt-2 flex items-center gap-1.5 px-3 py-2 text-xs font-bold text-white bg-[#1A1AE8] rounded-md hover:bg-[#1515c0] transition"><Plus size={14} />New Flow</button></div>}
